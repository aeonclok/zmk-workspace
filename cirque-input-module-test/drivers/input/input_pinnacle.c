...
static int pinnacle_init(const struct device *dev) {
    printk("Cirque driver init reached!\n");
    LOG_DBG("Test log");
    struct pinnacle_data *data = dev->data;
    const struct pinnacle_config *config = dev->config;
    int ret;
     ...
     // ADD THIS LINE:
    //k_msleep(900); // Delay to ensure Cirque is ready after power-up 

    int tries = 3;
    while (tries--) {
        ret = pinnacle_seq_read(dev, PINNACLE_FW_ID, fw_id, 2);
        if (ret == 0) break;
        LOG_WRN("Retrying FW ID read...");
        k_msleep(50);
    }
    if (ret < 0) {
        LOG_ERR("Failed to get the FW ID after retries");
        return ret;
    }
